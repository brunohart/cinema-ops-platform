...
| 2026-08-01 | README ordered for a four-minute first read — screenshot, failure-mode table, quickstart, agent safety, omissions; the argument demoted below the fold | lead with the thesis, as before | a reviewer who never reaches the proof table has not been persuaded of anything; four minutes is the budget, so the evidence goes above the argument |
...
| 2026-08-01 | three least-privilege Postgres roles: extractor writes bronze, transformer reads bronze and owns silver+gold, api reads gold (no PII columns) | one shared owner role | each role's blast radius is bounded at the grant boundary — a bug in dbt cannot corrupt bronze, a compromised serving credential cannot write anything and cannot read a name (ADR-016) |
| 2026-08-02 | README section 6 names what breaks at circuit scale with numbers, and pairs every omission with the first move I would make in a week | listing omissions with reasons but no remedy | an omission I cannot name a first move for is not a scoping decision, it is a gap I had not thought about; and the check that enforces it also fails the moment a "not built" claim outlives the build (VDE-56) |
| 2026-08-02 | AI practice stated above the fold with commit-date evidence and a machine proof (`scripts/prove_ai_practice.sh`) | describing the practice in prose, or leaving it below the fold with the pipeline section | a practice claimed is a claim; the order of commit one against the first pipeline commit is the only part of it a reader can check without trusting me (VDE-58) |
